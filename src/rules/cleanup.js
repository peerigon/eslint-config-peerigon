...
for (const [rule, config] of allRulesMap.entries()) {
  let oldRuleConfig = oldRulesMap.get(rule);

  if (oldRuleConfig === "warn" || oldRuleConfig?.at(0) === "warn") {
    oldRuleConfig = "error";
  } else if (oldRuleConfig?.at(0) === "warn") {
    oldRuleConfig[0] = "error";
  }
...
// console.log(oldRulesMap.size);

console.log(JSON.stringify(Object.fromEntries(oldRulesMap.entries()), null, 2));
